...
export default function WasteStreamTable({ isCs, rows, onChange }: Props) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h3>{isCs ? 'Toky odpadu' : 'Waste stream'}</h3>
      </div>
      ...
      <div className="gp-table-wrap">
        <table className="gp-table">
          <thead className="gp-table-head">
            <tr>
              <th className="gp-th gp-th-left pr-1">{isCs ? 'Tok odpadu' : 'Waste stream'}</th>
              <th className="gp-th gp-th-center w-4 px-0"></th>
              <th className="gp-th gp-th-center">{isCs ? 'Množství (kg/rok)' : 'Quantity (kg/year)'}</th>
              <th className="gp-th gp-th-center">{isCs ? 'Cíl / zpracování' : 'Destination / Treatment'}</th>
              <th className="gp-th gp-th-center">{isCs ? 'Recyklováno (kg)' : 'Recycled (kg)'}</th>
              <th className="gp-th gp-th-center">{isCs ? 'Energeticky využito vč. kompostu/AD (kg)' : 'Recovered incl. compost/AD/energy (kg)'}</th>
              <th className="gp-th gp-th-center">{isCs ? 'Podíl z celku (%)' : 'Share of total (%)'}</th>
              {/* Column removed – notes are shown only via tooltip */}
            </tr>
          </thead>
          <tbody>
            {rows.map((row) => {
              ...
              <tr key={row.id} className="gp-row">
                <td className="gp-td font-medium pr-1">{isCs ? row.labelCs : row.labelEn}</td>
                <td className="gp-td gp-td-center w-4 px-0">
                  <div className="flex justify-center">
                    <InfoTooltip label="Info" content={isCs ? row.notesCs : row.notesEn} />
                  </div>
                </td>
                ...
                {/* Notes column removed – notes are shown only via tooltip */}
              </tr>
            );
            })}

            <tr className="font-bold bg-stone-100 text-stone-900">
              <td className="gp-td uppercase">{isCs ? 'Celkem (kg)' : 'TOTAL (kg)'}</td>
